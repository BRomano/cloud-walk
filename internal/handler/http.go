...
func (handler *LogParserHTTPHandler) GetMatchesStatistics(w http.ResponseWriter, r *http.Request) {
	...
	logger := slog.With("game_id", gameIDStr, "endpoint", "GetMatchesStatistics")
...
func (handler *LogParserHTTPHandler) GetKillsByMeans(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	startTime := time.Now()
	...
	logger := slog.With("game_id", gameIDStr, "endpoint", "GetKillsByMeans")
	ctx := context.WithValue(r.Context(), "logger", logger)
	...
	statistics, err := handler.LogParserService.GetKillsByMeans(ctx, gameID, fileContent)
	if err != nil {
		logger.Error("could not get game statistics", "err", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	...
	logger.With("elapsed_time", time.Since(startTime))
	if err != nil {
		logger.Error("get kills by means finished with error", "err", err)
	} else {
		logger.Info("get kills by means finished")
	}
}
