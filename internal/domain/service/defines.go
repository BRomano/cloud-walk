...
func getDeathCauseByID(deathCauseID int) string {
	if deathCauseName, exists := modMap[deathCauseID]; exists {
		return deathCauseName
	}
	slog.Warn(fmt.Sprintf("unknow death cause id %d", deathCauseID))

	return getDeathCauseByID(MOD_UNKNOWN)
}
